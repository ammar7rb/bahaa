                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                            <div>
                                <h3 class="card-title mb-1">{{ translate('Customer_Purchase_Package') }}</h3>
                                <p class="fs-12 mb-0 text-muted">{{ translate('customer_purchase_limit_is_calculated_from_product_prices_only') }}</p>
                            </div>
                            @if($purchaseLimitSummary['has_active_package'])
                                <span class="badge badge-success text-bg-success">{{ translate('active') }}</span>
                            @else
                                <span class="badge badge-danger text-bg-danger">{{ translate('no_active_package') }}</span>
                            @endif
                        </div>

                        @if($purchaseLimitSummary['has_active_package'])
                            @php($subscription = $purchaseLimitSummary['subscription'])
                            <div class="row g-3 mb-4">
                                <div class="col-sm-6 col-xl-3">
                                    <div class="bg-section rounded-10 p-3 h-100">
                                        <p class="fs-12 mb-1">{{ translate('package') }}</p>
                                        <h4 class="mb-0">{{ $subscription?->package?->name ?? translate('not_found') }}</h4>
                                    </div>
                                </div>
                                <div class="col-sm-6 col-xl-3">
                                    <div class="bg-F0FFF3 rounded-10 p-3 h-100">
                                        <p class="fs-12 mb-1">{{ translate('available_limit') }}</p>
                                        <h4 class="mb-0 text-success">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $purchaseLimitSummary['available_limit'])) }}</h4>
                                    </div>
                                </div>
                                <div class="col-sm-6 col-xl-3">
                                    <div class="bg-FFF9F0 rounded-10 p-3 h-100">
                                        <p class="fs-12 mb-1">{{ translate('used_limit') }}</p>
                                        <h4 class="mb-0 text-warning">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $purchaseLimitSummary['used_limit'])) }}</h4>
                                    </div>
                                </div>
                                <div class="col-sm-6 col-xl-3">
                                    <div class="bg-section rounded-10 p-3 h-100">
                                        <p class="fs-12 mb-1">{{ translate('total_limit') }}</p>
                                        <h4 class="mb-0">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $purchaseLimitSummary['total_limit'])) }}</h4>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-sm-6 col-xl-3">
                                    <span class="d-block fs-12 text-muted">{{ translate('package_limit') }}</span>
                                    <strong>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $purchaseLimitSummary['package_limit'])) }}</strong>
                                </div>
                                <div class="col-sm-6 col-xl-3">
                                    <span class="d-block fs-12 text-muted">{{ translate('extra_credit_limit') }}</span>
                                    <strong>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $purchaseLimitSummary['extra_credit_limit'])) }}</strong>
                                </div>
                                <div class="col-sm-6 col-xl-3">
                                    <span class="d-block fs-12 text-muted">{{ translate('admin_adjustment_limit') }}</span>
                                    <strong>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $purchaseLimitSummary['admin_adjustment_limit'])) }}</strong>
                                </div>
                                <div class="col-sm-6 col-xl-3">
                                    <span class="d-block fs-12 text-muted">{{ translate('activated_at') }}</span>
                                    <strong>{{ $subscription?->activated_at ? date('d M Y h:i A', strtotime($subscription->activated_at)) : translate('no_data_found') }}</strong>
                                </div>
                            </div>

                            <form action="{{ route('admin.customer.purchase-limit-adjust', [$customer['id']]) }}" method="post" class="border rounded p-3 mb-4">
                                @csrf
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div class="select-wrapper">
                                            <select name="adjustment_type" class="form-select" required>
                                                <option value="add">{{ translate('add_limit') }}</option>
                                                <option value="subtract">{{ translate('subtract_limit') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label">{{ translate('amount') }}</label>
                                        <input type="number" name="amount" class="form-control" min="0.01" step="0.01" placeholder="{{ translate('Ex') }} : 200" required>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">{{ translate('note') }}</label>
                                        <input type="text" name="note" class="form-control" maxlength="1000" placeholder="{{ translate('reason') }}">
                                    </div>
                                    <div class="col-md-2">
                        ...
                        @else
                            <div class="alert alert-warning mb-4">
                                {{ translate('customer_does_not_have_an_active_purchase_package') }}
                            </div>
                        @endif
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100">
                                ...
                                <tbody>
                                @foreach($purchaseLimitTransactions as $transaction)
                                    <tr>
                                        <td>
                                            <div>{{ date('d M Y', strtotime($transaction->created_at)) }}</div>
                                            <div class="fs-12 text-muted">{{ date('h:i A', strtotime($transaction->created_at)) }}</div>
                                        </td>
                                        <td>
                                            <span class="badge badge-soft-info text-capitalize">{{ str_replace('_', ' ', $transaction->transaction_type) }}</span>
                                        </td>
                                        ...
                                        <td>{{ $transaction->reference ?? translate('no_data_found') }}</td>
                                        <td class="text-wrap max-w-360">{{ $transaction->note ?? translate('no_data_found') }}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                        ...
                        <div class="d-flex justify-content-end mt-3">
                            {!! $purchaseLimitTransactions->links() !!}
                        </div>
                        @if(count($purchaseLimitTransactions)==0)
                            @include('layouts.admin.partials._empty-state',['text'=>'no_transaction_found'],['image'=>'default'])
                        @endif
                    </div>
                </div>
            </div>
