            ...
            <div class="auth-wrapper-form text-center">
                <div class="__inline-20">
                    ...
                    <div class="mb-5">
                        ...
                        @isset($maskedPhone)
                            <p class="text-muted">{{ translate('Please_verify_your_phone') }}: {{ $maskedPhone }}</p>
                        @endisset
                    </div>

                    <form id="form-id" class="otp-form" action="{{ $formAction ?? route('vendor.auth.forgot-password.otp-verification') }}" method="post">
                        @csrf
                        ...
                        <div class="d-flex justify-content-center align-items-baseline flex-wrap gap-2">
                            <a class="btn btn-lg btn-block btn-secondary w-auto flex-grow-1"
                               href="{{ $backRoute ?? route('vendor.auth.forgot-password.index') }}">
                                {{ translate('back')}}
                            </a>
                    ...
                    @isset($resendRoute)
                        <form action="{{ $resendRoute }}" method="post" class="mt-3">
                            @csrf
...
<script>
    ...
    $(document).ready(function() {
        ...
        function checkOtpFilled() {
            ...
            $("#form-id [type=submit]").prop("disabled", !allFilled);
        }
    });
