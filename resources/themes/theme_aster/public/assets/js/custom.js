    ...
    coupons.forEach(span => {
        span.addEventListener('click', function () {
            ...
            navigator.clipboard.writeText(code).then(() => {
                toastr.success(document.getElementById('successfully-coupon-copied').getAttribute('data-text'));
            }).catch(err => {
...
$("#search-value").on("keyup", function () {
    ...
    $(".chat-list-item").each(function () {
        let name = $(this).find("h6").text().toLowerCase().trim();
    ...
    if (!hasResult) {
        if ($("#no-result").length === 0) {
            $(".chat-list.chatting-scrollbar").append(
                `<div id="no-result" class="text-center text-muted py-3">${$('#no-result-content').data('text')}</div>`
            );
        }
    } else {
        $("#no-result").remove();
