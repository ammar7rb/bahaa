...
class Helpers
{
    ...
    {
        return app(CustomerThreeStepShippingService::class)->getAvailableMethodsForSeller($seller_id, $type);
    }
...
//        $data['color_image'] = $colorImage;
    ...
    public static function get_seller_by_token($request)
    {
        ...
        if (count($token) > 1 && strlen($token[1]) > 30) {
            ...
            if (isset($seller)
                ...
                && ! app(SellerRegistrationVerificationService::class)->requiresPhoneVerification($seller)) {
                $data = $seller;
                $success = 1;
            }
        }
    ...
    public static function seller_sales_commission($seller_is, $seller_id, $order_total): int|string
    {
        ...
        return number_format(
            app(SellerCommissionService::class)->calculate(
                sellerType: $seller_is,
