...
trait UpdateClass
{
    ...
    public function getProcessAllVersionsUpdates(): void
    {
        ...
        $this->getInsertDataOfVersion('16.2');
    }
    ...
    public function getInsertDataOfVersion($versionNumber): void
    {
        ...
        if ($versionNumber == '16.2') {
            $deliveryMans = DeliveryMan::whereNull('full_phone')->get();
            foreach ($deliveryMans as $deliveryMan) {
                $deliveryMan->update([
                    'full_phone' => trim($deliveryMan->country_code) . trim($deliveryMan->phone)
                ]);
            }

            $firebaseOTPVerification = \App\Models\BusinessSetting::where('type', 'firebase_otp_verification')->first();
            $firebaseOTP = $firebaseOTPVerification ? json_decode($firebaseOTPVerification?->value, true) : [];
            if (!isset($firebaseOTP['web_api_key']) || empty($firebaseOTP['web_api_key'])) {
                \App\Models\BusinessSetting::where('type', 'firebase_otp_verification')->update([
                    'value' => json_encode(['status' => 0, 'web_api_key' => ''])
                ]);
            }

            \App\Models\SeoMetaInfo::where('seoable_type', 'App\Models\Category')->update(['index' => null, 'no_follow' => null]);

            $this->optimizeOrderDetailsProducts();

            foreach (User::whereNull('name')->get() as $user) {
                $user->update([
                    'name' => trim($user->f_name . ' ' . $user->l_name) ?: null,
                ]);
            }

            // Belt-and-suspenders rewrite of any admin_roles.module_access
            // rows still holding the pre-rename keys (pos_management, etc.).
            // The 2026_04_23 migration handles this for rows present at
            // migrate time; this catches rows added/restored later or any
            // case where the migration was rolled back and partially re-run.
            // Idempotent — exits cleanly when nothing is stale.
            Artisan::call('admin-roles:audit-module-keys', ['--fix' => true]);
        }
    ...
    function optimizeOrderDetailsProducts(): void
    {
        $orderDetails = \Illuminate\Support\Facades\DB::table('order_details')
            ->select('id', 'product_details', \Illuminate\Support\Facades\DB::raw('CHAR_LENGTH(product_details) as details_length'))
            ->orderByDesc('details_length')
            ->limit(200)
            ->get();
        ...
        foreach ($orderDetails as $detail) {
            ...
            $changed = false;
            if (isset($productDetails['order_details'])) {
                unset($productDetails['order_details']);
                $changed = true;
            }

            if (isset($productDetails['order_delivered'])) {
                unset($productDetails['order_delivered']);
                $changed = true;
            }

            if (isset($productDetails['seo_info'])) {
                unset($productDetails['seo_info']);
                $changed = true;
            }

            if ($changed) {
                \App\Models\OrderDetail::where('id', $detail->id)->update([
