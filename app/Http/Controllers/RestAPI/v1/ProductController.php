...
class ProductController extends Controller
{
    ...
    public function getHomepageFeaturedOffers(Request $request): JsonResponse
    {
        $limit = max(1, min((int) ($request['limit'] ?? 10), 50));
        $offset = max(1, (int) ($request['offset'] ?? 1));
        $products = ProductManager::getHomepagePromotedProductsQuery()
            ->paginate($limit, ['*'], 'page', $offset);
        ProductManager::markHomepagePromotedProducts($products->getCollection());
    ...
    public function getProductsFilter(Request $request): JsonResponse
    {
        ...
        $categories = isset($request?->category) && is_array($request?->category) ? $request?->category : json_decode($request?->category ?? '', true);
        $brand = isset($request?->brand) && is_array($request?->brand) ? $request?->brand : json_decode($request?->brand);
        ...
        $getCategories = $categories ? Category::where(['position' => 0])->whereIn('id', $categories)->get() : collect([]);
        $getSubCategories = $categories ? Category::where(['position' => 1])->whereIn('id', $categories)->get() : collect([]);
        $getSubSubCategories = $categories ? Category::where(['position' => 2])->whereIn('id', $categories)->get() : collect([]);
        ...
        $filteredCategoryIds = $getCategories->pluck('id')->toArray();
        ...
        $products = Product::active()->with(['rating', 'tags', 'clearanceSale' => function ($query) {
            ...
            ->when($request->has('category') && count($filteredCategoryIds) > 0, function ($query) use ($filteredCategoryIds, $filteredSubCategoryIds, $filteredSubSubCategoryIds) {
                return $query->whereIn('category_id', $filteredCategoryIds)
                    ->when(count($filteredSubCategoryIds) > 0, function ($query) use ($filteredSubCategoryIds) {
                        return $query->whereIn('sub_category_id', $filteredSubCategoryIds);
                    })->when(count($filteredSubSubCategoryIds) > 0, function ($query) use ($filteredSubSubCategoryIds) {
                        return $query->whereIn('sub_sub_category_id', $filteredSubSubCategoryIds);
                    });
            })
        ...
        if ($products['products']) {
            foreach ($products['products'] as $product) {
                $products_array[] = [
                    'id' => $product->id,
                    'name' => $product->name,
                    'is_search_promoted' => (bool) ($product->is_search_promoted ?? false),
                ];
            }
        }
    ...
    public function get_product_reviews(Request $request, $slug)
    {
        $product = Product::withCount('reviews')->where('slug', $slug)->first();
        if (!$product) {
            return response()->json([
                'errors' => ['code' => 'product-001', 'message' => translate('product_not_found')]
            ], 404);
        ...
        $averageRating = count($product->rating) > 0 ? number_format($product->rating[0]->average, 2, '.', ' ') : 0;
        $reviews = Review::with(['customer', 'product', 'reply'])->where(['product_id' => $product['id']])
            ->latest('updated_at')
            ->paginate(($request['limit'] ?? 10), ['*'], 'page', ($request['offset'] ?? 1));

        $ratingGroupCount = Review::where(['product_id' => $product['id']])
        ...
        return response()->json([
            ...
            'limit' => (int)$request['limit'],
            'offset' => (int)$request['offset'],
            'group-wise-rating' => $ratingGroupCount,
    ...
    {
        $methods = app(CustomerThreeStepShippingService::class)->getAllVisibleMethods();
