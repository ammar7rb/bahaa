...
class RegisterController extends Controller
{
    ...
    public function submitRegisterData(CustomerRegistrationRequest $request): JsonResponse|RedirectResponse
    {
        ...
        if (isset($request['referral_code']) && !empty($request['referral_code']) && empty($referUser)) {
            if ($request->ajax()) {
                return response()->json([
                    'error' => translate('Referral Code is invalid.'),
                ]);
            }

            Toastr::error(translate('Referral Code is invalid.'));
            return back();
        }
