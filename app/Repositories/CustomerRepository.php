...
class CustomerRepository implements CustomerRepositoryInterface
{
    ...
    public function getCountForDashboard(?Carbon $startDate, ?Carbon $endDate): int
    {
        return $this->user->whereNot('email', 'walking@customer.com')
        ...
        $query = $this->user
            ...
            ->when(isset($filters['avoid_walking_customer']) && $filters['avoid_walking_customer'] == 1, function ($query) {
                return $query->where(function ($q) {
                    $q->whereNull('email')->orWhere('email', '!=', 'walking@customer.com');
                });
            })
            ->when(!empty($orderBy), function ($query) use ($orderBy) {
                $query->orderBy(array_key_first($orderBy), array_values($orderBy)[0]);
