...
$(".clearance-product-add-submit").on("click", function () {
    ...
    ajaxSetupToken();
    // Helper: admin runs Bootstrap 5 only — `$(...).modal('hide'|'show')`
    // is BS4 jQuery sugar that doesn't exist on BS5, so the close call
    // silently no-op'd and the modal stayed open after a successful add.
    // Use the BS5 vanilla API directly.
    function getProductAddModal() {
        let el = document.getElementById('product-add-modal');
        if (!el) return null;
