...
$("#inputChecked, #privacyChecked").change(function () {
    if ($("#inputChecked").is(":checked") && $("#privacyChecked").is(":checked")) {
...
// --- Text Descrition toggle bases on char limit ---
$(function () {
    let seeMoreText = $('#see-more-less').data('see-more') || 'See More';
    let seeLessText = $('#see-more-less').data('see-less') || 'See Less';

    $('.review-wrapper').each(function () {

        let wrapper = $(this);
        let textEl = wrapper.find('.review-des');
        let button = wrapper.find('.see_more_btn');
        let limit = parseInt(wrapper.attr('data-char-limit')) || 150;

        let fullText = $.trim(textEl.text());

        if (fullText.length <= limit) return;

        let shortText = fullText.substring(0, limit) + '...';

        textEl.data('full', fullText);
        textEl.data('short', shortText);

        textEl.text(shortText);
        button.text(seeMoreText).removeClass('d-none');
    });
    ...
    $(document).on('click', '.see_more_btn', function () {
        ...
        if (btn.hasClass('open')) {
            textEl.text(textEl.data('short'));
            btn.removeClass('open').text(seeMoreText);
        } else {
            textEl.text(textEl.data('full'));
            btn.addClass('open').text(seeLessText);
        }
